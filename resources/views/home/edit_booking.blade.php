...
<body>
        ...
        <form action="{{ route('update_booking', $booking->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="gender" id="Male" value="Male"  {{ $booking->gender === 'Male' ? 'checked' : '' }} required>
                    <label class="form-check-label" for="Male">Male</label>
                </div>
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="gender" id="Female" value="Female" {{ $booking->gender === 'Female' ? 'checked' : '' }} required>
            ...
            <div class="btn-container">
                <a class="btn btn-dark btn-back" href="{{route('all_booking')}}">Back</a>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // ดึงค่า move_in_date จากฐานข้อมูล (ในรูปแบบ YYYY-MM-DD)
            const databaseDate = '{{ $booking->move_in_date ?? '' }}';

            // ถ้ามีค่าที่ได้จากฐานข้อมูล
            if (databaseDate) {
                // สร้างวัตถุ Date จากค่าในฐานข้อมูล
                const defaultDate = new Date(databaseDate);
                // เพิ่ม 1 วันเพื่อให้เป็นวันถัดไป
                defaultDate.setDate(defaultDate.getDate() + 1);
                // แปลงเป็นรูปแบบวันที่ที่ถูกต้อง (YYYY-MM-DD)
                const formattedDefaultDate = defaultDate.toISOString().substring(0, 10);
                // กำหนดค่าให้กับ input field
                moveInDateInput.value = formattedDefaultDate;
            }
        });
        $(document).ready(function() {
            $('form').submit(function(e) {
                e.preventDefault();
                Swal.fire({
                    title: "Are you sure to edit this booking?",
                    showDenyButton: true,
                    ...
                    if (result.isConfirmed) {
                        ...
                        Swal.fire("Edit Booking are not success ", "", "info");
                    }
                });
            });

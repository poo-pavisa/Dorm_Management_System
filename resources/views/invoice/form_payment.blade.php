...
<body>
    <div class="container">
        <div class="promptpay-container">
            <div>
                ...
                <div class="thai-qr">
                    <img src="{{ asset('images/qrcode/qrcode_' . $invoice->invoice_ref . '.png') }}" alt="PromptPay QR Code">
            ...
            <p class="deposit-text">{{ number_format($invoice->total_amount, 2, '.', ',') }} BAHT</p>
        </div>
        <a href="{{ asset('images/qrcode/qrcode_' . $invoice->invoice_ref . '.png') }}" class="btn btn-outline-info btn-download" target="_blank" download>DOWNLOAD QRCODE</a>  
    </div>
    ...
    @if($errors)
        ...
        @endforeach
    @endif
        <form action="{{route('payment_invoice',$invoice->id)}}" method="POST" enctype="multipart/form-data" class="slip-form">
            @csrf
                ...
                <a class="btn btn-outline-dark" href="{{route('invoice_detail' , $invoice->id)}}">Back</a>
        ...
        $(document).ready(function() {
            ...
            if ($('.alert-success').length) {
                Swal.fire({
                    ...
                    setTimeout(function() {
                        window.location.href = "{{ route('all_invoice') }}";
                    }, 1000);
                });
            }
        });
